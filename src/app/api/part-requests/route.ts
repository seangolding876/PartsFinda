...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'getMakes') {
      const makesResult = await query('SELECT id, name FROM makes ORDER BY name');
      return NextResponse.json({ success: true, data: makesResult.rows });
    }
    ...
    if (action === 'getModels') {
      ...
      if (!uuidRegex.test(makeId)) return NextResponse.json({ success: false, error: 'Invalid make ID format' }, { status: 400 });

      const modelsResult = await query('SELECT id, name FROM models WHERE make_id = $1 ORDER BY name', [makeId]);
      return NextResponse.json({ success: true, data: modelsResult.rows });
    }
